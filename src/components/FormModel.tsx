...
import { FaTrash, FaPenSquare, FaTimes, FaPen, FaPlus } from "react-icons/fa";
...
const forms: { [key: string]: (type: "create" | "update", data?: any) => JSX.Element } = {
    ...
    // attandance: (type, data) => <AttendanceForm type={type} data={data} />,
  ...
  return (
    <>
        <button className={`${size} flex items-center justify-center rounded-full ${bgColor}`} onClick={() => setOpen(true)}>
            {type === "create" ? <FaPlus className="text-gray-200" size={18}/> : <FaPen className="text-gray-500" size={18}/>}
        </button>
        {open && (
